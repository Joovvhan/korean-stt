...
def get_spectrogram_feature(filepath):
    ...
    # logger.info('Shape of sig: {}'.format(sig.shape))

    _, _, Zxx = sp.signal.stft(sig,
                               fs=SAMPLE_RATE,
                               nperseg=nsc,
                               noverlap=nov)
    ...
    mel_filters = librosa.filters.mel(sr=SAMPLE_RATE, n_fft=nsc, n_mels=n_mels)
    mel_specgram = np.matmul(mel_filters, Sxx)

    log_mel_specgram = 20 * np.log10(np.maximum(mel_specgram, eps))
    norm_log_mel_specgram = (log_mel_specgram + db_ref) / db_ref

    # logger.info('Shape of norm_log_mel_specgram: {}'.format(norm_log_mel_specgram.shape))

    feat = torch.FloatTensor(norm_log_mel_specgram)
...
class BaseDataset(Dataset):
    def __init__(self, wav_paths, script_paths, korean_script_paths, bos_id=1307, eos_id=1308):
        self.wav_paths = wav_paths
        self.script_paths = script_paths
        self.korean_script_paths = korean_script_paths
