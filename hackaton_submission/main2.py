...
def bind_model(modelA, modelB, optimizerA=None, optimizerB=None):
    def load(filename, **kwargs):
        state = torch.load(os.path.join(filename, 'modelA.pt'))
        modelA.load_state_dict(state['model'])
        if 'optimizer' in state and optimizerA:
            optimizerA.load_state_dict(state['optimizer'])

        state = torch.load(os.path.join(filename, 'modelB.pt'))
        modelB.load_state_dict(state['model'])
        if 'optimizer' in state and optimizerB:
            optimizerB.load_state_dict(state['optimizer'])

        print('Model loaded')

    def save(filename, **kwargs):
        state = {
            'model': modelA.state_dict(),
    ...
    def infer(wav_path):
        ...
        logit = modelA(input_variable=input, input_lengths=None, teacher_forcing_ratio=0)
...
def main():
    ...
    bind_model(net, net_B, net_optimizer, net_B_optimizer)
    ...
    for epoch in range(args.max_epochs):
        ...
        logger.info("Total Evaluation Reference CER: {}".format(train_cer_ref))
