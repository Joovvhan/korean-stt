...
# My Decoder
class CTC_Decoder_v2(nn.Module):
    def __init__(self, H, D_out, num_chars):
        super(CTC_Decoder_v2, self).__init__()
        self.fc_embed = nn.Linear(H, H)
        self.relu_embed = torch.nn.ReLU()
...
class Mel2SeqNet_v2(nn.Module):
    def __init__(self, D_in, H, D_out, num_chars, device):
        ...
        self.encoder = Encoder(D_in, H).to(device)
        self.decoder = CTC_Decoder_v2(H, D_out, num_chars).to(device)
...
class Mel2SeqNet_v3(nn.Module):
    def __init__(self, D_in, H, D_out, num_chars, device):
        ...
        self.encoder = Encoder_v3(D_in, H).to(device)
        self.decoder = CTC_Decoder_v3(H, D_out, num_chars).to(device)
...
class Encoder_General(nn.Module):
...
class CTC_Decoder_General(nn.Module):
    def __init__(self, H, D_out, num_chars, num_layers):
        ...
        self.gru_layers = nn.ModuleList([nn.GRU(H, D_out, batch_first=True)] + [nn.GRU(D_out, D_out, batch_first=True) for i in range(num_layers - 1)])
...
class Residual_GRU(nn.Module):
    def __init__(self, in_D, out_D, bidirectional=False, batch_first=True):
        ...
        self.gru = nn.GRU(in_D, out_D, bidirectional=bidirectional, batch_first=batch_first)
...
class Encoder_General_Residual(nn.Module):
    def __init__(self, D_in, H, num_layers):
        super(Encoder_General_Residual, self).__init__()
        self.fc = torch.nn.Linear(D_in, H)
        self.relu = torch.nn.ReLU()
        self.dropout = nn.Dropout(p=0.2)

        self.gru_layers = nn.ModuleList([Residual_GRU(H, int(H / 2), bidirectional=True) for i in range(num_layers)])
...
class CTC_Decoder_General_Residual(nn.Module):
    def __init__(self, H, D_out, num_chars, num_layers):
        super(CTC_Decoder_General_Residual, self).__init__()
        self.fc_embed = nn.Linear(H, H)
        self.relu_embed = torch.nn.ReLU()
        self.dropout_embed = nn.Dropout(p=0.2)

        self.gru_layers = nn.ModuleList([nn.GRU(H, D_out, batch_first=True)] + [Residual_GRU(D_out, D_out) for i in range(num_layers - 1)])
...
class Mel2SeqNet_General_Residual(nn.Module):
    def __init__(self, D_in, H, D_out, num_chars, num_layers, device):
        ...
        self.encoder = Encoder_General_Residual(D_in, H, num_layers).to(device)
        self.decoder = CTC_Decoder_General_Residual(H, D_out, num_chars, num_layers).to(device)
